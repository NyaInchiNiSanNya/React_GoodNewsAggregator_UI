...
let store = {
    ...
    dispatch(action) {
        this._state.login=loginReducer(this._state.content,action);
        this._renderTree(this._state, this.dispatch.bind(store));
    }
}

export const updateLoginActionCreator= (newLogin) =>{
    return {
        type:'update-login',
        newLogin: newLogin
    }
